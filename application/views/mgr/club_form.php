                                    ...
                                    <div class="col-md-<?= $item[6] ?> col-sm-<?= $item[7] ?>">
                                        <div class="form-group">
                                            ...
                                            <? elseif ($item[2] == "select") : ?>
                                                <select class="form-control select2" name="<?= $item[1] ?>" <?= (isset($can_edit) && !$can_edit) ? " disabled" : "" ?> id="<?= $item[1] ?>">
                                                    <?
                                                    $class = "$item[1]";
                                                    if (isset($item[8])) {
                                                        foreach ($select[$item[1]] as $option) {
                                                            echo '<option class=' . $class . ' value="' . $option[$item[8][0]] . '"';
                                        ...
                                        <?
                                                if (isset($pics) && is_array($pics)) {
                                                    foreach ($pics as $pic) {
                                                        $c = $pic['url'];
                                        ?>
                                                <div class="<?= (isset($type) && $type == "edit") ? 'col-lg-3 col-md-4 col-sm-4 col-xs-6' : 'col-lg-2 col-md-3 col-sm-4 col-xs-6' ?>">
                                                    <a data-fancybox="gallery_<?= $item[1] ?>" href="<?= base_url() . $c ?>"><img src="<?= base_url() . $c ?>" class="thumbnail" style="width:100%;"></a>
                                                    <? if (isset($can_edit) && !$can_edit) : ?><? else : ?>
                                                    <button type="button" class="btn btn-sm btn-danger del-btn" onclick="del_multi_img(this, '<?= $c ?>', '<?= $item[1] ?>');">
                                                        <span class="fa fa-fw ti-trash"></span>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#category").on('change', function(e) {
            ...
            $('#classify').empty();
            $.each(classify[sel].classify, function(item) {
                $('#classify').append("<option value='" + this.id + "'>" + this.title + "</option>");
            });
        });

        $("input[name=q1]").attr('placeholder', '請輸入審核問題(若沒有則空白)');
        $("input[name=q2]").attr('placeholder', '請輸入審核問題(若沒有則空白)');
        $("input[name=q3]").attr('placeholder', '請輸入審核問題(若沒有則空白)');
    });
