...
/*
	...
	public function __construct()
	{
		...
		$this->data['active'] = 'SEMINAR';
		$this->action = base_url() . 'mgr/seminar/';
	}
	...
	public function local()
	{
		$this->data = array_merge($this->data, array(
			'sub_active'              => 'SEMINAR_LOCAL',
			'parent'                  => '',
			// 'parent_link'             => base_url() . 'mgr/event/main',
			'custom_del_url'          => base_url() . 'mgr/seminar/local_del',
			'title'                   => '研討會公告列表',
			'action'                  => $this->action,
	...
	public function data()
	{
		...
		$data = $this->Seminar_model->get_local_list($syntax, $order_by, $page, $this->page_count);
		$html = "";
		foreach ($data['list'] as $item) {
			$html .= $this->load->view("mgr/items/seminar_local_item", array(
				"item" =>    $item,
			), TRUE);
		}
	...
	{
		$seminar = $this->Seminar_model->get_data($id);
		...
		if ($_POST) {
			...
			// !d(!$banner_del_syntax);
			// exit;
			if ($this->Seminar_model->edit($id, $data)) {
				if ($banner_data)
					$this->Seminar_model->add_banner($banner_data);
				if ($banner_del_syntax)
					$this->Seminar_model->del_banner($banner_del_syntax);
				$this->js_output_and_redirect("編輯成功", base_url() . "mgr/seminar/$seminar[type]/");
			} else {
				$this->js_output_and_back("發生錯誤");
			}
		} else {
			$this->data['pics'] = $this->Seminar_model->get_banner($id);
			$this->data['type'] = 'edit';
			$this->data['param'] = $this->set_data_to_param($this->param, $seminar);
			$this->data['title'] = '編輯公告【' . $seminar['name'] . '】';
			$this->data['sub_active'] = 'SEMINAR_LOCAL' ;

			$this->data['parent'] = '公告列表';
			$this->data['parent_link'] = base_url() . "mgr/seminar/local";

			$this->data['action'] = base_url() . "mgr/seminar/local_edit/" . $id;
			$this->data['submit_txt'] = "確認編輯";

			$this->data['select']['category'] = $this->Seminar_model->get_hobby_seminar_classify();
			$this->data['classify'] = json_encode($this->data['select']['category']);
			$this->data['select']['is_private'] = array(['id' => 1, 'title' => '公開'], ['id' => 0, 'title' => '不公開']);
			// TODO: 新欄位還沒串
			// !d($this->data);
			// exit;
			$this->load->view("mgr/seminar_form", $this->data);
		}
	}
	...
	public function local_del()
	{
		...
		if ($this->Seminar_model->edit($id, array('is_delete' => 1))) {
	...
	// ----------------------------------------------------------------文章列表 start
	public function post_list($seminar_id)
	{
		$seminar = $this->Seminar_model->get_data($seminar_id);
		$this->data = array_merge($this->data, array(
			'sub_active'              => 'SEMINAR_LOCAL',
			'parent'                  => '文章列表' ,
			'parent_link'             =>  base_url() . 'mgr/seminar/local' ,
			'custom_data_url'          => base_url() . 'mgr/seminar/post_data/' . $seminar_id,
			'title'                   => '【' . $seminar['name'] . '】的文章列表',
			'action'                  => $this->action,
...
/*
	public function post_data($seminar_id)
	{
		$page        = ($this->input->post("page"))			? $this->input->post("page")		: 1;
		$search      = ($this->input->post("search"))		? $this->input->post("search")		: '';
		$order       = ($this->input->post("order"))		? $this->input->post("order")		: 0;
		...
		$syntax	= "P.is_delete=0 AND post_at='seminar' AND relation_id='$seminar_id'";
		if ($status != 'ALL')
		...
		$lists = $this->Seminar_model->get_post_list($syntax, $order_by, intval($page), $page_count = 20);
