...
class Huntground extends Base_Controller
{

	/* for <thead></thead> - Start */
	private $local_th_title         = ['#', '公告名稱', '公告內容','上傳者', '動作'];
	private $local_th_width         = ['', '350px', '500px', '', '', '150px'];
...
/*
	...
	public function __construct()
	{
		...
		$this->data['active'] = 'HUNTGROUND';
		$this->action = base_url() . 'mgr/huntground/';
	}
	...
	public function local()
	{
		$this->data = array_merge($this->data, array(
			'sub_active'              => 'HUNTGROUND_LOCAL',
			'parent'                  => '',
			// 'parent_link'             => base_url() . 'mgr/event/main',
			'custom_del_url'          => base_url() . 'mgr/huntground/local_del',
			'title'                   => '公告列表',
			'action'                  => $this->action,
	...
	public function data()
	{
		...
		$data = $this->Club_model->get_local_list($syntax, $order_by, $page, $this->page_count);
		$html = "";
		foreach ($data['list'] as $item) {
			$html .= $this->load->view("mgr/items/club_local_item", array(
				"item" =>    $item,
			), TRUE);
		}
	...
	public function local_add(){
		...
		if ($_POST) {
			...
			// !d(!$banner_del_syntax);
			// exit;
			if ($this->Club_model ->add($data) !== FALSE) {
				var_dump($data);
			...
			//$this->data['pics'] = $this->Club_model->get_banner($id);
			$this->data['type'] = 'local';
			$this->data['param'] = $this->param;
			$this->data['title'] = '新增公告';
			$this->data['sub_active'] =  'HUNTGROUND_LOCAL';

			$this->data['parent'] = '公告列表';
			$this->data['parent_link'] = base_url() . "mgr/huntground/local";

			$this->data['action'] = base_url() . "mgr/huntground/local_add/";
			$this->data['submit_txt'] = "新增";

			$this->data['select']['category'] = $this->Club_model->get_hobby_club_classify();
			...
			$this->load->view("mgr/club_form", $this->data);
		}
	
	}
	public function local_edit($id)
	{
		$club = $this->Club_model->get_data($id);
		require('C:/xampp/htdocs/wundoo/vendors/autoload.php');
		if ($_POST) {
			$data = $this->process_post_data($this->param);
			//TODO: 新欄位還沒串
			...
			// !d(!$banner_del_syntax);
			// exit;
			if ($this->Club_model->edit($id, $data)) {
				if ($banner_data)
					$this->Club_model->add_banner($banner_data);
				if ($banner_del_syntax)
					$this->Club_model->del_banner($banner_del_syntax);
				$this->js_output_and_redirect("編輯成功", base_url() . "mgr/huntground/$club[type]/");
			} else {
				$this->js_output_and_back("發生錯誤");
			}
		} else {
			$this->data['pics'] = $this->Club_model->get_banner($id);
			$this->data['type'] = 'edit';
			$this->data['param'] = $this->set_data_to_param($this->param, $club);
			$this->data['title'] = '編輯公告【' . $club['name'] . '】';
			$this->data['sub_active'] =  'HUNTGROUND_LOCAL';

			$this->data['parent'] = '公告列表';
			$this->data['parent_link'] = base_url() . "mgr/huntground/local";

			$this->data['action'] = base_url() . "mgr/huntground/local_edit/" . $id;
	...
	public function local_del()
	{
		...
		if ($this->Club_model->edit($id, array('is_delete' => 1))) {
	...
	// ----------------------------------------------------------------文章列表 start
	public function post_list($club_id)
	{
		$club = $this->Club_model->get_data($club_id);
		$this->data = array_merge($this->data, array(
			'sub_active'              => 'HUNTGROUND_LOCAL',
			'parent'                  => '文章列表' ,
			'parent_link'             => ($club['type'] == 'local') ? base_url() . 'mgr/huntground/local' : base_url() . 'mgr/huntground/hobby',
			'custom_data_url'          => base_url() . 'mgr/huntground/post_data/' . $club_id,
			'title'                   => '【' . $club['name'] . '】的文章列表',
			'action'                  => $this->action,
	...
	public function post_data($club_id)
	{
		$page        = ($this->input->post("page"))			? $this->input->post("page")		: 1;
		$search      = ($this->input->post("search"))		? $this->input->post("search")		: '';
		$order       = ($this->input->post("order"))		? $this->input->post("order")		: 0;
		...
		$syntax	= "P.is_delete=0 AND post_at='club' AND relation_id='$club_id'";
		if ($status != 'ALL')
		...
		$lists = $this->Club_model->get_post_list($syntax, $order_by, intval($page), $page_count = 20);
