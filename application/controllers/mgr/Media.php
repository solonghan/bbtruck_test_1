...
class media extends Base_Controller
{
    ...
    private $local_th_title         = ['#', '標題', '類型','訪問單位','照片','外部連結', '上傳時間', '動作'];
    private $local_th_width         = ['150px', '200px', '', '350px','','', '',''];
   ...
   // private $type                   = ['BRIDAL-GOWNS', 'EVENING-GOWNS'];
    ...
    private $param = [
        //																							md 		sm
        ["專訪類型",		 	"type", 			"text", 			"", 		TRUE, 	"", 	4, 		12],
        ["專訪單位",             "hash_tag",             "text",             "",         FALSE,     "",    4,         12, 2 / 1],
        
        ["標題",                "title",             "textarea_plain",             "",         FALSE,     "",    4,         12, 2 / 1],
        ...
        ["圖片",             "img",             "img_multi_without_crop",             "",         FALSE,     "",     0.667,         12, 2 / 1],
    ...
    public function __construct()
    {
        ...
        $this->action = base_url() . 'mgr/media/';
    }
    ...
    public function index()
    {
        ...
        $this->data = array_merge($this->data, array(
            ...
            'custom_del_url'          => base_url() . 'mgr/media/local_del',
            'title'                   => '圖片列表',
            'action'                  => $this->action,
            ...
            'tool_btns'               => [
                ...
                ['add media', base_url() . 'mgr/media/add', 'btn-primary'],
            ],
        ));
    ...
    public function data()
    {
        ...
        $data = $this->media_model->get_local_list($syntax, $order_by, $page, $this->page_count);
        $html = "";
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/media_local_item", array(
                "item" =>    $item,
            ), TRUE);
        }
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            var_dump($data);
            if ($this->media_model->add($data) !== FALSE) {


                $this->js_output_and_redirect("新增成功", base_url() . "mgr/media/index/");
            } else {
            ...
            $this->data['sub_active'] =  'media';

            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/media/index/";

            $this->data['action'] = base_url() . "mgr/media/add/";
    ...
    public function edit($id)
    {
        ...
        require('./vendors/autoload.php');
        //require('C:/xampp/htdocs/new_wedding/vendors/autoload.php');
        $media = $this->media_model->get_data($id);
        //var_dump($media);
        //exit;
        if ($_POST) {
            $data = $this->process_post_data($this->param);

            if ($this->media_model->edit($id, $data)) {

                $this->js_output_and_redirect("編輯成功", base_url() . "mgr/media/");
            } else {
                $this->js_output_and_back("發生錯誤");
            }
        } else {
            //$this->data['pics'] = $this->media_model->get_banner($id);
            $this->data['type'] = 'edit';
            $this->data['param'] = $this->set_data_to_param($this->param, $media);
            $this->data['title'] = '編輯照片';
            $this->data['sub_active'] =  'media';

            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/media/";
            $this->data['select']['type'] = array(['id' => 'EVENING-GOWNS', 'title' => 'EVENING-GOWNS'], ['id' => 'BRIDAL-GOWNS', 'title' => 'BRIDAL-GOWNS']);
            
            $this->data['action'] = base_url() . "mgr/media/edit/" . $id;
            $this->data['submit_txt'] = "確認編輯";
            //$this->data['select']['type'] = array(['id' => 'all_media', 'title' => 'all_media'], ['id' => 'ready_to_wear', 'title' => 'ready_to_wear'],['id' => 'customize', 'title' => 'customize'],['id' => 'media', 'title' => 'media']);
            // TODO: 新欄位還沒串
            //$this->data['select']['category'] = $this->media_model->get_hobby_media_classify();
    ...
    public function local_del($id)
    {
        ...
        if ($this->media_model->edit($id, array('is_delete' => 1))) {
            //$this->output(TRUE, 'success');
            $this->js_output_and_redirect("刪除成功", base_url() . "mgr/media/");
        } else {
    ...
    // ----------------------------------------------------------------文章列表 start
    public function post_list($media_id)
    {
        $media = $this->media_model->get_data($media_id);
        $this->data = array_merge($this->data, array(
            'sub_active'              => 'media_MGR_LOCAL',
            'parent'                  => ($media['type'] == 'local') ? '在地獵場列表' : '同好獵場',
            'parent_link'             => ($media['type'] == 'local') ? base_url() . 'mgr/huntground/local' : base_url() . 'mgr/huntground/hobby',
            'custom_data_url'          => base_url() . 'mgr/media/post_data/' . $media_id,
            'title'                   => '【' . $media['name'] . '】的文章列表',
            'action'                  => $this->action,
    ...
    public function post_data($media_id)
    {
        $page        = ($this->input->post("page"))            ? $this->input->post("page")        : 1;
        $search      = ($this->input->post("search"))        ? $this->input->post("search")        : '';
        $order       = ($this->input->post("order"))        ? $this->input->post("order")        : 0;
        ...
        $syntax    = "P.is_delete=0 AND post_at='media' AND relation_id='$media_id'";
        if ($status != 'ALL')
        ...
        $lists = $this->media_model->get_post_list($syntax, $order_by, intval($page), $page_count = 20);
    ...
    public function hobby_add()
    {
        if ($_POST) {
            ...
            $this->load->view("mgr/media_form", $this->data);
        }
    }
