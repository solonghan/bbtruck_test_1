...
class Timeline extends Base_Controller
{
    ...
    private $type                   = ['all_collection', 'ready_to_wear', 'customize', 'shop', 'timeline'];
    ...
    private $param = [
       ...
       // ["標題1",             "title1",             "text",             "",         TRUE,     "",     4,         12],
        ["左側圖一",             "left_img1",             "img_multi_without_crop",             "",         TRUE,     "",     4,         12],
        ["左側圖二",             "left_img2",             "img_multi_without_crop",             "",         TRUE,     "",     4,         12],
       
        //["中間內文",             "center_content",             "textarea",             "",         TRUE,     "",     4,         12],
        ["右測圖一",             "right_img1",             "img_multi_without_crop",             "",         FALSE,     "",     12,         12, 2 / 1],
        ["右側圖二",             "right_img2",             "img_multi_without_crop",             "",         FALSE,     "",     12,         12, 2 / 1],
        
        ["大圖",             "big_img",             "img_multi_without_crop",             "",         TRUE,     "",     12,         12, 2 / 1],
        
        ["頁面介紹",             "introduction",             "textarea_plain",             "",         TRUE,     "",     4,         12],
    ...
    public function __construct()
    {
        ...
        $this->action = base_url() . 'mgr/timeline/';
    }
    ...
    public function index()
    {
        ...
        $timeline = $this->Timeline_model->get_data(1);
        
        $this->data = array_merge($this->data, array(
            'sub_active'              => 'ABOUT',
            'parent'                  => '',
            // 'parent_link'             => base_url() . 'mgr/event/main',
            'custom_del_url'          => base_url() . 'mgr/timeline/local_del',
            'title'                   => '圖片列表',
            'action'                  => $this->action,
            ...
            'tool_btns'               => [
                ...
                //['add img', base_url() . 'mgr/timeline/add', 'btn-primary'],
            ],
        ));
        ...
        if ($_POST) {
            ...
            if ($this->Timeline_model->edit(1, $data)) {

                $this->js_output_and_redirect("編輯成功", base_url() . "mgr/timeline/");
            } else {
            ...
            $this->data['sub_active'] =  'TIMELINE';

            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/timeline/";

            $this->data['action'] = base_url() . "mgr/timeline/" ;
    ...
    public function data()
    {
        ...
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/timeline_local_item", array(
                "item" =>    $item,
            ), TRUE);
        }
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->Home_model->add($data) !== FALSE) {
                ...
                $this->js_output_and_redirect("編輯成功", base_url() . "mgr/timeline/index/");
            } else {
                $this->js_output_and_back("發生錯誤");
            }
        } else {
            ...
            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/timeline/index/";

            $this->data['action'] = base_url() . "mgr/timeline/add/";
            ...
            $this->load->view("mgr/timeline_form", $this->data);
        }
    }
    ...
    public function edit($id)
    {
        ...
        require('./vendors/autoload.php');
       // require('C:/xampp/htdocs/new_wedding/vendors/autoload.php');
        $timeline = $this->Home_model->get_data($id);
        //var_dump($timeline);
        //exit;
        if ($_POST) {
            $data = $this->process_post_data($this->param);
            ...
            $this->data['action'] = base_url() . "mgr/timeline/edit/" . $id;
            $this->data['submit_txt'] = "確認編輯";
            //$this->data['select']['type'] = array(['id' => 'all_collection', 'title' => 'all_collection'], ['id' => 'ready_to_wear', 'title' => 'ready_to_wear'],['id' => 'customize', 'title' => 'customize'],['id' => 'timeline', 'title' => 'timeline']);
            // TODO: 新欄位還沒串
            //$this->data['select']['category'] = $this->Timeline_model->get_hobby_timeline_classify();
    ...
    public function local_del($id)
    {
        ...
        if ($this->Timeline_model->edit($id, array('is_delete' => 1))) {
            //$this->output(TRUE, 'success');
            $this->js_output_and_redirect("刪除成功", base_url() . "mgr/timeline/");
        } else {
    ...
    // ----------------------------------------------------------------文章列表 start
    public function post_list($timeline_id)
    {
        $timeline = $this->timeline_model->get_data($timeline_id);
        $this->data = array_merge($this->data, array(
            'sub_active'              => 'TIMELINE_MGR_LOCAL',
            'parent'                  => ($timeline['type'] == 'local') ? '在地獵場列表' : '同好獵場',
            'parent_link'             => ($timeline['type'] == 'local') ? base_url() . 'mgr/huntground/local' : base_url() . 'mgr/huntground/hobby',
            'custom_data_url'          => base_url() . 'mgr/timeline/post_data/' . $timeline_id,
            'title'                   => '【' . $timeline['name'] . '】的文章列表',
            'action'                  => $this->action,
    ...
    public function post_data($timeline_id)
    {
        $page        = ($this->input->post("page"))            ? $this->input->post("page")        : 1;
        $search      = ($this->input->post("search"))        ? $this->input->post("search")        : '';
        $order       = ($this->input->post("order"))        ? $this->input->post("order")        : 0;
        ...
        $syntax    = "P.is_delete=0 AND post_at='timeline' AND relation_id='$timeline_id'";
        if ($status != 'ALL')
        ...
        $lists = $this->timeline_model->get_post_list($syntax, $order_by, intval($page), $page_count = 20);
    ...
    public function hobby_data()
    {
        ...
        $data = $this->timeline_model->get_local_list($syntax, $order_by, $page, $this->page_count);
        $html = "";
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/timeline_local_item", array(
                "item" =>    $item,
    ...
    public function hobby_add()
    {
        if ($_POST) {
            ...
            // TODO: 新欄位還沒串
            if ($this->timeline_model->edit($id, $data)) {
