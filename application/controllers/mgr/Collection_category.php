...
class Collection_category extends Base_Controller
{

    /* for <thead></thead> - Start */
    private $local_th_title         = ['#', '類別', '上傳時間', '動作'];
    private $local_th_width         = ['50px', '200px', '100px', '50px'];
    ...
    private $param = [
        ...
        ["category",             "name",             "text",             "",         FALSE,     "",     0.667,         12, 2 / 1],
    ...
    public function __construct()
    {
        ...
        $this->action = base_url() . 'mgr/collection_category/';
    }
    ...
    public function index()
    {
        ...
        $this->data = array_merge($this->data, array(
            'sub_active'              => 'COLLECTION_CATEGORY',
            'parent'                  => '',
            // 'parent_link'             => base_url() . 'mgr/event/main',
            'custom_del_url'          => base_url() . 'mgr/collection_category/local_del',
            'title'                   => '圖片列表',
            'action'                  => $this->action,
            ...
            'tool_btns'               => [
                ...
                ['add collection', base_url() . 'mgr/collection_category/add', 'btn-primary'],
            ],
        ));
    ...
    public function data()
    {
        ...
        $data = $this->Category_model->get_local_list($syntax, $order_by, $page, $this->page_count);
        $html = "";
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/collection_category_local_item", array(
                "item" =>    $item,
            ), TRUE);
        }
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->Category_model->add($data) !== FALSE) {
                ...
                $this->js_output_and_redirect("新增成功", base_url() . "mgr/collection_category/index/");
            } else {
            ...
            $this->data['title'] = '新增類別';
            $this->data['sub_active'] =  'COLLECTION_CATEGORY';

            $this->data['parent'] = '類別列表';
            $this->data['parent_link'] = base_url() . "mgr/collection_category/index/";

            $this->data['action'] = base_url() . "mgr/collection_category/add/";
    ...
    public function edit($id)
    {
        ...
        //require('C:/xampp/htdocs/new_wedding/vendors/autoload.php');
        $collection = $this->Category_model->get_category_data($id);
        //var_dump($collection);
        //exit;
        if ($_POST) {
            $data = $this->process_post_data($this->param);
            ...
            if ($this->Category_model->edit($id, $data)) {
                ...
                $this->js_output_and_redirect("編輯成功", base_url() . "mgr/collection_category/");
            } else {
                $this->js_output_and_back("發生錯誤");
            }
        } else {
            ...
            $this->data['parent_link'] = base_url() . "mgr/collection_category/";
            //$this->data['select']['type'] = array(['id' => 'EVENING-GOWNS', 'title' => 'EVENING-GOWNS'], ['id' => 'BRIDAL-GOWNS', 'title' => 'BRIDAL-GOWNS']);
            
            $this->data['action'] = base_url() . "mgr/collection_category/edit/" . $id;
    ...
    public function local_del($id)
    {
        ...
        if ($this->Category_model->edit($id, array('is_delete' => 1))) {
            //$this->output(TRUE, 'success');
            $this->js_output_and_redirect("刪除成功", base_url() . "mgr/collection_category/");
        } else {
