...
class Acdemic extends Base_Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		$this->is_mgr_login();
		$this->load->model('Acdemic_model');

		$this->data['active'] = 'ACDEMIC';
		$this->action = base_url() . 'mgr/acdemic/';
	}
	...
	public function local()
	{
		$this->data = array_merge($this->data, array(
			...
			'custom_del_url'          => base_url() . 'mgr/acdemic/local_del',
			'title'                   => '學術公告列表',
			'action'                  => $this->action,
	...
	public function data()
	{
		...
		$data = $this->Acdemic_model->get_local_list($syntax, $order_by, $page, $this->page_count);
		$html = "";
		foreach ($data['list'] as $item) {
			$html .= $this->load->view("mgr/items/acdemic_local_item", array(
				"item" =>    $item,
			), TRUE);
		}
	...
	{
		$acdemic = $this->Acdemic_model->get_data($id);
		...
		if ($_POST) {
			...
			// !d(!$banner_del_syntax);
			// exit;
			if ($this->Acdemic_model->edit($id, $data)) {
				if ($banner_data)
					$this->Acdemic_model->add_banner($banner_data);
				if ($banner_del_syntax)
					$this->Acdemic_model->del_banner($banner_del_syntax);
				$this->js_output_and_redirect("編輯成功", base_url() . "mgr/acdemic/$acdemic[type]/");
			} else {
				$this->js_output_and_back("發生錯誤");
			}
		} else {
			$this->data['pics'] = $this->Acdemic_model->get_banner($id);
			$this->data['type'] = 'edit';
			$this->data['param'] = $this->set_data_to_param($this->param, $acdemic);
			$this->data['title'] = '編輯學術公告【' . $acdemic['name'] . '】';
			$this->data['sub_active'] =  'ACDEMIC_LOCAL';

			$this->data['parent'] = '在地獵場列表';
			$this->data['parent_link'] = base_url() . "mgr/acdemic/local";

			$this->data['action'] = base_url() . "mgr/acdemic/local_edit/" . $id;
			$this->data['submit_txt'] = "確認編輯";

			$this->data['select']['category'] = $this->Acdemic_model->get_hobby_acdemic_classify();
			$this->data['classify'] = json_encode($this->data['select']['category']);
			$this->data['select']['is_private'] = array(['id' => 1, 'title' => '公開'], ['id' => 0, 'title' => '不公開']);
			// TODO: 新欄位還沒串
			// !d($this->data);
			// exit;
			$this->load->view("mgr/acdemic_form", $this->data);
		}
	}
	...
	public function local_del()
	{
		...
		if ($this->acdemic_model->edit($id, array('is_delete' => 1))) {
	...
	// ----------------------------------------------------------------文章列表 start
	public function post_list($acdemic_id)
	{
		$acdemic = $this->acdemic_model->get_data($acdemic_id);
		$this->data = array_merge($this->data, array(
			'sub_active'              => 'ACDEMIC_LOCAL',
			'parent'                  => ($acdemic['type'] == 'local') ? '在地獵場列表' : '同好獵場',
			'parent_link'             => ($acdemic['type'] == 'local') ? base_url() . 'mgr/huntground/local' : base_url() . 'mgr/huntground/hobby',
			'custom_data_url'          => base_url() . 'mgr/acdemic/post_data/' . $acdemic_id,
			'title'                   => '【' . $acdemic['name'] . '】的文章列表',
			'action'                  => $this->action,
	...
	public function post_data($acdemic_id)
	{
		$page        = ($this->input->post("page"))			? $this->input->post("page")		: 1;
		$search      = ($this->input->post("search"))		? $this->input->post("search")		: '';
		$order       = ($this->input->post("order"))		? $this->input->post("order")		: 0;
		...
		$syntax	= "P.is_delete=0 AND post_at='acdemic' AND relation_id='$acdemic_id'";
		if ($status != 'ALL')
		...
		$lists = $this->acdemic_model->get_post_list($syntax, $order_by, intval($page), $page_count = 20);
