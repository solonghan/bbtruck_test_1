...
class Designer_model extends Base_Model {
...
*/
	...
	public function edit($home_id, $data){
		return $this->db->where(array("id"=>$home_id))->update($this->designer_table, $data);
	}
	...
	public function add($data){
       ...
       // var_dump($data['type']);
		...
		$res = $this->db->insert($this->designer_table, $data);
	...
	public function get_data($home_id){
		return $this->db->get_where($this->designer_table, array("id"=>1))->row_array();
	}
	...
	{
		$total = $this->db->select()->from($this->designer_table)->where($syntax)->get()->num_rows();
		$total_page = ceil($total / $page_count);


        
		$list = $this->db->select()
			->from($this->designer_table)
			->where($syntax)
			->limit($page_count, ($page - 1) * $page_count)
			->order_by($order_by)
			->get()
	...
	public function get_designer_view(){
		...
		$data=array(
			...
			'quote2_img'		=>	$res['quote2_img'],
			'quote3_title'		=>	$res['quote3_title'],
			'quote3_img'		=>	$res['quote3_img'],
