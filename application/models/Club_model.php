...
class Club_model extends Base_Model {
	...
	public function discuss_hot($club_id, $value = 1){
		$club = $this->get_data($club_id);

		$discuss_hot = intval($club['discuss_hot']) + intval($value);

		$this->discuss_hot_log($club_id, $value);

		return $this->edit($club['id'], array(
			"discuss_hot" => $discuss_hot
		));
	}

	public function discuss_hot_log($club_id, $value = 1){
		$today = date("Y-m-d");
		$club_discuss = 0;
		$syntax = array(
			"club_id" => $club_id,
		...
		$data = $this->db->get_where($this->club_discuss_log_table, $syntax)->row_array();
		if ($data == null) {
			$syntax['club_discuss'] = $value;
			return $this->db->insert($this->club_discuss_log_table, $syntax);
		}else{
			$club_discuss = intval($data['club_discuss']) + $value;
			return $this->db->where($syntax)->update($this->club_discuss_log_table, array("club_discuss"=>$club_discuss));
		}
	}

	public function dropout_club($club_id, $user_id){
		$syntax = array(
			"club_id"	=>	$club_id,
			"user_id"	=>	$user_id
		);
		$this->dropout_log($syntax, "myself");
		return $this->db->delete($this->user_club_related_table, $syntax);
	}

	public function dropout_log($syntax, $dropout_by){
		$data = $this->db->get_where($this->user_club_related_table, $syntax)->row_array();
		if ($data == null) return;
		$this->db->insert($this->user_club_dropout_table, array(
		...
		$this->update_club_people($data['club_id'], 1, 'minus');
	}

	//guard apply
	public function review_guard_apply($apply_id, $status){
		$apply_data = $this->db->get_where($this->club_guard_apply_table, array("id"=>$apply_id))->row_array();
		if ($apply_data == null) return FALSE;
		if ($status != 'success' && $status != 'reject') return FALSE;

		if ($this->db->where(array("id"=>$apply_id))->update($this->club_guard_apply_table, array("status"=>$status))) {
			if ($status == 'success') {
				return $this->db->where(array(
					"club_id"	=>	$apply_data['club_id'],
					"user_id"	=>	$apply_data['user_id']
				))->update($this->user_club_related_table, array(
	...
	public function apply_guard_list($club_id){
		$list = $this->db->select("U.*, G.id as apply_id, G.social_exp, G.reason, G.role")
						 ->from($this->club_guard_apply_table." G")
						 ->join($this->user_table." U", "U.id = G.user_id", "left")
						 ->where(array("G.club_id"=>$club_id, "G.status"=>"pending"))
						 ->get()->result_array();
		$data = array();
	...
	public function apply_guard($club_id, $user_id, $data){
		$syntax = array(
			"club_id"	=>	$club_id,
			"user_id"	=>	$user_id
		);
		if ($this->db->get_where($this->club_guard_apply_table, $syntax)->num_rows() > 0) return FALSE;
		$data = array_merge($data, $syntax);
		return $this->db->insert($this->club_guard_apply_table, $data);
	}

	public function get_club_member_report_list($club_id, $page = 1, $search = ""){
		$syntax = "R.club_id = '{$club_id}' AND T.is_delete = 0";
		if ($search != "") {
		...
		$list = $this->db->select("R.id as report_id, R.type, R.post_id, R.reason, R.create_date, R.status as report_status, U.id as user_id, T.*,
						...
						, '') as post_title
			")
						 ->from($this->club_user_report_table." R")
	...
	public function club_report_action($report_id, $action){
		return $this->db->where(array("id"=>$report_id))->update($this->club_user_report_table, array("status"=>$action));
	}

	public function report_member($user_id, $target_id, $club_id, $reason){
		return $this->db->insert($this->club_user_report_table, array(
			"type"      =>	"user",
			"user_id"   =>	$user_id,
			"target_id" =>	$target_id,
			"club_id"   =>	$club_id,
	...
	public function report_club_post($user_id, $post_id, $club_id, $reason){
	...
	public function club_kickoff_member($user_id, $club_id){
		$syntax = array(
			"user_id" =>	$user_id,
			"club_id" =>	$club_id
		);
		$this->dropout_log($syntax, "club");
		return $this->db->delete($this->user_club_related_table, $syntax);
	}

	public function check_user_club_role($user_id, $club_id, $role){
		$syntax = array(
			"user_id" =>	$user_id,
			"club_id" =>	$club_id
		);
	...
	public function change_user_role($user_id, $club_id, $role){
		$syntax = array(
			"user_id"	=>	$user_id,
			"club_id"	=>	$club_id
		);
		if ($this->db->get_where($this->user_club_related_table, $syntax)->num_rows() > 0) {
			return $this->db->where($syntax)->update($this->user_club_related_table, array("role"=>$role));
		}else{
			return FALSE;
		}
	}

	public function change_user_muted($user_id, $club_id, $is_muted, $date, $reason){
		$syntax = array(
			"user_id"	=>	$user_id,
			"club_id"	=>	$club_id
		);
		if ($this->db->get_where($this->user_club_related_table, $syntax)->num_rows() > 0) {
			return $this->db->where($syntax)->update($this->user_club_related_table, array("is_muted"=>$is_muted));
		}else{
			return FALSE;
		}
	}
/*
	public function check_user_can_speak($club_id, $user_id){
		$syntax = array(
			"user_id"	=>	$user_id,
			"club_id"	=>	$club_id
		);
		$r = $this->db->get_where($this->user_club_related_table, $syntax)->row_array();
		...
		if ($r['is_muted'] == 0) {
			...
			if (strtotime($r['muted_date']) < strtotime(date("Y-m-d H:i:s"))) {
				$this->db->where($syntax)->update($user_club_related_table, array("is_muted"=>0));
				return TRUE;
			}else{
				return FALSE;
			}
	...
	public function get_club_member($club_id, $page = 1, $search = ""){
		$syntax = "R.club_id = '{$club_id}' AND U.is_delete = 0";
		if ($search != "") {
	...
	public function get_unreview_member($club_id, $search){
		$syntax = "R.status = 'pending' AND R.club_id = '{$club_id}' and U.is_delete = 0";
		if ($search != "") {
			$field = ["U.atid", "U.email", "U.mobile", "U.username", "U.nickname"];
			$search_syntax = "";
			foreach ($field as $f) {
		...
		$list = $this->db->select("U.*, R.*")
						 ->from($this->user_club_apply_table." R")
						 ->join($this->user_table." U", "U.id = R.user_id", "left")
						 ->where($syntax)
						 ->order_by("R.create_date DESC")
						 ->get()->result_array();
	...
	public function join_club_for_waiting_review($user_id, $club_id, $data){
		$syntax = array(
			"user_id"	=>	$user_id,
			"club_id"	=>	$club_id
		);
		if ($this->db->get_where($this->user_club_apply_table, $syntax)->num_rows() <= 0) {
			$data = array_merge($data, $syntax);
			return $this->db->insert($this->user_club_apply_table, $data);
		}else{
			// $data = $this->db->get_where($this->user_club_apply_table, $syntax)->row_array();
			// if ($data['status'] == 'reject') return FALSE;
			$data = array_merge($data, $syntax);
			$data['status'] = 'pending';
			return $this->db->where($syntax)->update($this->user_club_apply_table, $data);
		}
	}
...
/*
	public function get_club_privilege_btns($club_id, $user_id){
		$join_club = $this->user_has_join_club($user_id, $club_id);

		$manager_privilege = $this->check_user_club_role($user_id, $club_id, "manager");
		$guard_privilege = $this->check_user_club_role($user_id, $club_id, "guard");

		$add_post_btn_enabled = TRUE;
		$add_post_btn_text = "我要發文";
		if($join_club == "joined"){
			if (!$this->check_user_can_speak($club_id, $user_id)) {
		...
		$add_post = array(
			...
			"hidden"  =>	($join_club == "joined")?FALSE:TRUE,
		...
		}else if ($join_club == "pending") {
			$apply_join['enabled'] = FALSE;
			$apply_join['hidden'] = FALSE;
			$apply_join['text'] = "審核中";
		}else if ($join_club == "reject") {
			$apply_join['enabled'] = FALSE;
			$apply_join['hidden'] = TRUE;
		}

		$dropout = array(
			"enabled" =>	($join_club == "joined")?TRUE:FALSE,
			"hidden"  =>	($join_club == "joined")?FALSE:TRUE,
			"text"    =>	"退出此獵場"
		);
...
*/
	public function get_club_post_classify_list($club_id, $id_as_key = FALSE){
		$syntax = array("is_delete" => 0);
		if ($club_id != "") {
			$syntax['club_id'] = $club_id;
		}
		if ($id_as_key) {
			return $this->db->get_where($this->club_post_classify_table, $syntax)->result_array("id");	
		}
		$data = $this->db->select("id, title, can_edit")->get_where($this->club_post_classify_table, $syntax)->result_array();
	...
	public function user_has_join_club($user_id, $club_id){
		//joined pending reject none
		$joined = $this->db->get_where($this->user_club_related_table, array("user_id"=>$user_id, "club_id"=>$club_id))->row_array();
		if ($joined != null) return "joined";

		$apply = $this->db->get_where($this->user_club_apply_table, array("user_id"=>$user_id, "club_id"=>$club_id))->row_array();
		if ($apply != null) {
			return $apply['status'];
		}else{
			return "none";
	...
	public function user_join_club($user_id, $club_arr){
		$data = array();
		foreach ($club_arr as $c) {
			...
			if ($this->db->get_where($this->user_club_related_table, $c_related)->num_rows() <= 0) {
				$data[] = $c_related;
				$this->update_club_people($c, 1, 'plus');
			}
		}
	...
	public function update_club_people($club_id, $cnt, $action = 'plus'){
		$club = $this->db->get_where($this->club_table, array("id"=>$club_id))->row_array();
		$people = intval($club['people']);
		if ($action == "plus") {
			$people++;
		...
		$this->db->where(array("id"=>$club_id))->update($this->club_table, array("people"=>$people));
	}
	...
	public function update_club_post_classify($club_id, $classify){
		$msg = "";
		foreach ($classify as $item) {
			if (!array_key_exists('action', $item)) continue;
			if ($item['action'] == "add") {
				$r = $this->db->insert($this->club_post_classify_table, array(
					"club_id"  =>	$club_id,
					"title"    =>	$item['title'],
					"can_edit" =>	1
				));
				...
				$c = $this->db->get_where($this->club_post_classify_table, array("id"=>$item['id']))->row_array();
				if ($c['can_edit'] == 1) {
					$r= $this->db->where(array("id"=>$item['id']))->update($this->club_post_classify_table, array(
						"title"	=>	$item['title']
					));
	...
	public function add($data, $is_multi = FALSE){
		if ($is_multi) {
			return $this->db->insert_batch($this->club_table, $data);
		}else{
