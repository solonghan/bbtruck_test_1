...
class Customized_model extends Base_Model {
...
*/
	...
	public function edit($home_id, $data){
		return $this->db->where(array("id"=>$home_id))->update($this->customized_table, $data);
	}
	...
	public function add($data){
       ...
       // var_dump($data['type']);
		...
		$res = $this->db->insert($this->customized_table, $data);
	...
	public function get_data($home_id){
		return $this->db->get_where($this->customized_table, array("id"=>1))->row_array();
	}
	...
	{
		$total = $this->db->select()->from($this->customized_table)->where($syntax)->get()->num_rows();
		$total_page = ceil($total / $page_count);


        
		$list = $this->db->select()
			->from($this->customized_table)
			->where($syntax)
			->limit($page_count, ($page - 1) * $page_count)
			->order_by($order_by)
			->get()
	...
	public function get_customized_view(){
