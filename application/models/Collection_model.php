...
*/
	...
	{
		return $this->db->where(array("id" => $home_id))->update($this->collection_table, $data);
	}
	...
	public function add($data)
	{
		...
		// var_dump($data['type']);
		//    exit;
		$res = $this->db->insert($this->collection_table, $data);
	...
	{
		return $this->db->get_where($this->collection_table, array("id" => $home_id))->row_array();
	}
	...
	{
		$total = $this->db->select()->from($this->collection_table)->where($syntax)->get()->num_rows();
	...
	public function get_collection_category()
	{
		...
		foreach ($res as $r) {
			...
			$data[] = array(
				'upper_case'	=>	$r['name'],
				'lower_case'	=>	strtolower($r['name'])
			);
		}


		//$r=array_merge($data,$add);
		// !d($r);
		// exit;
	...
	public function get_post($type)
	{
		...
		$str = "select * from collection where is_delete=0 and type='$type_upper' order by sort asc ";
		$res = $this->db->query($str)->result_array();
	...
	public function get_rtw_carousel($type)
	{
		//!d($type);

		$str="select * from ready_carousel_top where is_delete=0 and type='$type'";
		$res=$this->db->query($str)->result_array();
...
/////
	public function get_type_name(){
		$type_data = array();
		foreach ($this->db->get_where($this->collection_category_table, array( "is_delete" => 0))->result_array() as $b) {
			$type_data[] = array(
				"id"    => $b['name'],
				"title" => $b['name'],
