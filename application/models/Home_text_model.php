...
class Home_text_model extends Base_Model {
...
*/
	...
	public function edit($home_id, $data){
		return $this->db->where(array("id"=>$home_id))->update('home_text', $data);
	}
	...
	public function add($data){
       ...
       // var_dump($data['type']);
		...
		$res = $this->db->insert($this->home_text_table, $data);
	...
	public function get_data($home_id){
		return $this->db->get_where($this->home_text_table, array("id"=>$home_id))->row_array();
	}
	...
	{
		$total = $this->db->select()->from($this->home_text_table)->where($syntax)->get()->num_rows();
		$total_page = ceil($total / $page_count);


        
		$list = $this->db->select()
			->from($this->home_text_table)
			->where($syntax)
			->limit($page_count, ($page - 1) * $page_count)
			->order_by($order_by)
			->get()
	...
	public function get_content(){
		$str="select * from home_text where is_delete=0 and type = 'shop' limit 1";
	...
	public function get_title(){
		$str="select * from home_text where is_delete=0 ";
		$res=$this->db->query($str)->result_array();
		$data=array();
		...
		foreach($res as $r){
			...
			$data[]=array(
				'no'			=>	$r['no'],
				'desktop_title'	=>	$r['desktop_title'],
				'mobile_title'	=>	$r['mobile_title']
