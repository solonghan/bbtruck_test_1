...
class Timeline_model extends Base_Model {
...
*/
	...
	public function edit($home_id, $data){
		return $this->db->where(array("id"=>$home_id))->update($this->timeline_table, $data);
	}
	...
	public function add($data){
       ...
       // var_dump($data['type']);
		...
		$res = $this->db->insert($this->timeline_table, $data);
	...
	public function get_data($home_id){
		return $this->db->get_where($this->timeline_table, array("id"=>1))->row_array();
	}
	...
	{
		$total = $this->db->select()->from($this->timeline_table)->where($syntax)->get()->num_rows();
		$total_page = ceil($total / $page_count);


        
		$list = $this->db->select()
			->from($this->timeline_table)
			->where($syntax)
			->limit($page_count, ($page - 1) * $page_count)
			->order_by($order_by)
			->get()
	...
	public	function get_timeline_view(){
		...
		$data=array(
			...
			'quote'			=>	$res['quote'],
			'left_img1'		=>	$res['left_img1'],
			'left_img2'		=>	$res['left_img2'],
			'center_content'=>	$res['center_content'],
			'right_img1'	=>	$res['right_img1'],
			'right_img2'	=>	$res['right_img2'],
			'big_img'		=>	$res['big_img'],
